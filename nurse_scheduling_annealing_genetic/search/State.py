...
class State:
    ...
    def generate(self):
        ...
        for i in range(len(self.scheduling)):
            generated.append(State(self.n_nurses, self.n_periods, self.swap_bit(i)))
    ...
    def generate_random(self):
        ...
        """
        return State(self.n_nurses, self.n_periods, self.swap_bit(randint(0, len(self.scheduling))))

    def mutate(self, index):
        """
        Mutates the State changing one of it's ith scheduling value
        """
        self.scheduling = self.swap_bit(index)
    ...
    def __str__(self):
        state = ''
        for i in range(self.n_nurses):
            state += 'e{:02g}: '.format(i + 1) + '[' + self.scheduling[
                                                       i * self.n_periods: (i + 1) * self.n_periods] + ']' + '\n'
